...
export async function exportarInformePDF({ secciones, alcanceTexto, trabajadores, vehiculos, incidencias, baseMap }) {
  ...
  if (secciones.trab) {
    tabla("Trabajadores", trabajadores, [
      { label: "Nombre", width: 130, get: (t) => t.nombre },
      { label: "DNI", width: 65, get: (t) => t.dni || "—" },
      { label: "ID", width: 45, get: (t) => t.id_personal || "—" },
      { label: "Puesto", width: 150, get: (t) => [t.titulo, t.puesto_trabajo].filter(Boolean).join(" · ") || "—" },
      { label: "Contrato", width: 70, get: (t) => t.tipo_contrato || "—" },
      { label: "Antigüedad", width: 65, get: (t) => (t.fecha_antiguedad ? new Date(t.fecha_antiguedad).toLocaleDateString("es-ES") : "—") },
      { label: "Baja", width: 35, get: (t) => (t.de_baja ? "Sí" : "No") },
      { label: "Base", width: 105, get: (t) => t.base?.nombre || baseMap[t.base?.id || t.base_id]?.nombre || "—" },
      { label: "Área", width: 90, get: (t) => baseMap[t.base?.id || t.base_id]?.areaNombre || "—" },
    ]);
  }
