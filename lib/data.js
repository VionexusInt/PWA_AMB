...
export async function getArea(areaId) {
  ...
  let trab = [], veh = [], inc = [], incResueltas = [];
  if (baseIds.length) {
    const [t, v, i, ir] = await Promise.all([
      supabase.from("trabajadores").select("id, base_id").in("base_id", baseIds).eq("en_bolsa", false),
      ...
      supabase.from("incidencias").select("base_id").eq("resuelta", true).in("base_id", baseIds),
    ]);
    trab = t.data || []; veh = v.data || []; inc = i.data || []; incResueltas = ir.data || [];
  }
  const bases = (basesR.data || []).map((b) => ({
    ...
    resueltas: incResueltas.filter((x) => x.base_id === b.id).length,
  }));
...
export async function getBase(baseId) {
  const [baseR, trabR, vehR, incR] = await Promise.all([
    ...
    supabase.from("trabajadores").select("*").eq("base_id", baseId).eq("en_bolsa", false).order("nombre"),
...
export async function getVehiculo(vehiculoId) {
  ...
  const [asigR, incR, plantillaR, vehsR] = await Promise.all([
    ...
    supabase.from("trabajadores").select("id, nombre").eq("base_id", baseId).eq("en_bolsa", false).order("nombre"),
...
export const getTodosTrabajadores = () =>
  supabase.from("trabajadores").select("*, base:bases(nombre)").eq("en_bolsa", false).order("nombre");
export const getTodosVehiculos = () =>
  supabase.from("vehiculos").select("*, base:bases(nombre)").order("matricula");

// --- TODAS LAS INCIDENCIAS (seguridad + comité de empresa, con su origen) ---
export async function getTodasIncidencias() {
  const [segR, empR] = await Promise.all([
    supabase
      .from("incidencias")
      .select("*, adjuntos(id), base:bases(id,nombre), trabajador:trabajadores(id,nombre), vehiculo:vehiculos(id,matricula)")
      .order("fecha", { ascending: false }),
    supabase.from("incidencias_empresa").select("*").order("fecha", { ascending: false }),
  ]);
  if (segR.error) return { data: null, error: segR.error };

  const seguridad = (segR.data || []).map((i) => ({ ...i, _origenEmpresa: false }));
  let empresa = empR.data || [];

  if (empresa.length) {
    const ids = empresa.map((e) => e.id);
    const { data: adjs } = await supabase
      .from("css_adjuntos")
      .select("id, registro_id")
      .eq("tabla", "incidencias_empresa")
      .in("registro_id", ids);
    const conteo = {};
    (adjs || []).forEach((a) => { conteo[a.registro_id] = (conteo[a.registro_id] || 0) + 1; });
    empresa = empresa.map((e) => ({
      ...e,
      _origenEmpresa: true,
      adjuntos: Array.from({ length: conteo[e.id] || 0 }),
    }));
  }

  const data = [...seguridad, ...empresa].sort((a, b) => new Date(b.fecha) - new Date(a.fecha));
  return { data, error: null };
}

// Resolver (o reabrir) una incidencia con nota de texto + fecha, de cualquiera de las dos tablas.
// Sube también los archivos de la resolución (si hay) al mismo sitio donde vive esa incidencia.
export async function resolverIncidenciaConNota(origenEmpresa, id, { resuelta, texto, fecha, archivos = [] }) {
  const tabla = origenEmpresa ? "incidencias_empresa" : "incidencias";
  const payload = { resuelta };
  if (resuelta) {
    payload.fecha_resolucion = fecha ? new Date(fecha).toISOString() : new Date().toISOString();
    payload.resolucion_texto = texto || null;
  } else {
  ...
  for (const f of archivos) {
    if (origenEmpresa) await subirCSSAdjuntoMulti("incidencias_empresa", id, f);
    else await subirAdjunto(id, f);
  }
}
...
export async function getDatosInforme() {
  const [areasR, trabR, vehR, incR] = await Promise.all([
    supabase.from("areas").select("id, nombre, bases(id, nombre)").order("nombre"),
    supabase.from("trabajadores").select("*, base:bases(id, nombre, area_id)").eq("en_bolsa", false).order("nombre"),
...
export const borrarTipoIncidencia = (nombre) =>
  supabase.from("tipos_incidencia").delete().eq("nombre", nombre);
...
export async function getIncidenciasEmpresa() {
  ...
  if (items.length) {
    ...
    const conteo = {};
    (adjs || []).forEach((a) => { conteo[a.registro_id] = (conteo[a.registro_id] || 0) + 1; });
    items = items.map((i) => ({ ...i, adjuntos: Array.from({ length: conteo[i.id] || 0 }) }));
  }
  return items;
}
...
// ============================================
export const getProtocolos = (tabla) =>
  supabase.from(tabla).select("*").order("fecha", { ascending: false, nullsFirst: false });

export const crearProtocolo = (tabla, campos) =>
  supabase.from(tabla).insert([campos]).select().single();

export const actualizarProtocolo = (tabla, id, campos) =>
  supabase.from(tabla).update(campos).eq("id", id).select().single();

export const borrarProtocolo = (tabla, id) =>
  supabase.from(tabla).delete().eq("id", id);

// ============================================
// BOLSA DE TRABAJADORES
// ============================================
export async function mandarABolsa(trabajadorId, baseIdActual) {
  const { error } = await supabase
    .from("trabajadores")
    .update({ en_bolsa: true, base_id_origen: baseIdActual })
    .eq("id", trabajadorId);
  if (error) throw error;
}

export async function getTrabajadoresBolsa() {
  const { data, error } = await supabase
    .from("trabajadores")
    .select("*, incidencias(id)")
    .eq("en_bolsa", true)
    .order("nombre");
  if (error) throw error;
  return data || [];
...
export async function readmitirTrabajador(trabajadorId, nuevaBaseId) {
  const { error } = await supabase
    .from("trabajadores")
    .update({ en_bolsa: false, base_id: nuevaBaseId, base_id_origen: null })
    .eq("id", trabajadorId);
  if (error) throw error;
}

// ============================================
// PROPUESTAS (en ambos comités)
// ============================================
export const getPropuestas = (tabla) =>
  supabase.from(tabla).select("*").order("fecha", { ascending: false, nullsFirst: false });

export const crearPropuesta = (tabla, campos) =>
  supabase.from(tabla).insert([campos]).select().single();

export const actualizarPropuesta = (tabla, id, campos) =>
  supabase.from(tabla).update(campos).eq("id", id).select().single();

export const marcarPropuestaRealizada = (tabla, id, realizada) =>
  supabase.from(tabla).update({
    realizada,
    fecha_realizacion: realizada ? new Date().toISOString().split("T")[0] : null,
  }).eq("id", id);

export const borrarPropuesta = (tabla, id) =>
  supabase.from(tabla).delete().eq("id", id);
