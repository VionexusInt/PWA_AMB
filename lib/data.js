...
// --- BASE: datos + trabajadores + vehículos + incidencias + resumen del área ---
export async function getBase(baseId) {
  const [baseR, trabR, vehR, incR] = await Promise.all([
    supabase.from("bases").select("id, nombre, tipo, area_id, area:areas(nombre)").eq("id", baseId).single(),
  ...
  // Totales de TODA el área a la que pertenece esta base
  let areaResumen = null;
  const areaId = baseR.data.area_id;
  const basesArea = await supabase.from("bases").select("id").eq("area_id", areaId);
  const ids = (basesArea.data || []).map((b) => b.id);
  if (ids.length) {
    const [tc, vc] = await Promise.all([
      supabase.from("trabajadores").select("id", { count: "exact", head: true }).in("base_id", ids),
      supabase.from("vehiculos").select("id", { count: "exact", head: true }).in("base_id", ids),
    ]);
    areaResumen = {
      nombre: baseR.data.area?.nombre || "Área",
      n_trab: tc.count || 0,
      n_veh: vc.count || 0,
    };
  }
...
export const addSugerencia = (tipo, texto, dispositivo) =>
  supabase.from("sugerencias").insert({ tipo, texto, dispositivo });
